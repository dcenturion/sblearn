...
// require_once("class/lib_upload.php");
...
function GuardaMensajeChatDDB($Id_Edu_Almacen,$DCTimeHour,$Entity,$Edu_Chat_Crud_Mensaje,$User){
	
	$ConxDDB = DynamoDbClient::factory([
		'region'  => 'us-east-2',
			'key'    => AWS_ACCES_KEY,
	...
	try {
		...
		$iterator = $ConxDDB->getIterator('Scan', array(
			'TableName' => 'sala',
			'ScanFilter' => array(
				
				'Id_Alias_Exteno' => array(
					'AttributeValueList' => array(
						array('N' => $Id_Edu_Almacen)
					),
		...
		foreach ($iterator as $item) {
			// echo $item['Id_Sala']['N'] . "\n";
			$Correlativo = $item['Id_Sala']['N'] + 1;
		}				
		
		if(empty($Correlativo)){
			
			$result = $ConxDDB->putItem(array(
				'TableName' => 'sala',
				'Item' => array(
					'Id_Sala'      => array('N' =>  $Id_Edu_Almacen ),
					'Id_Alias_Exteno'      => array('N' =>  $Id_Edu_Almacen ),
					'Empresa'    => array('N' => $Entity),
					'Id_Sala_Platform_Exter'    => array('N' => 1),
					'Estado'   => array('S' => 'Activo'),
					'Fecha_Hora_Creacion'   => array('S' => $DCTimeHour),
					'Fecha_Hora_Actualizacion' => array('S' => $DCTimeHour)
				)
			));										
			...
			$result = $ConxDDB->putItem(array(
				'TableName' => 'sala_conversacion',
				'Item' => array(
					'Id_Sala'      => array('N' =>  $Id_Sala ),
					'Id_Sala_Conversacion'    => array('S' => (string)$lastInsertId),
					'Mensaje'   => array('S' => $Edu_Chat_Crud_Mensaje),
					'Nombres'   => array('S' => $Row->Nombre),
					'Id_Usuario'   => array('S' => $User),
					'User_Platform_Exter'   => array('N' => $User),
					'Fecha_Hora_Creacion'   => array('S' => $DCTimeHour),
					'Estado'   => array('S' => 'Activo'),
					'Fecha_Hora_Actualizacion' => array('S' => $DCTimeHour)
				)
...
*//*
function LeerMensajeChatDDB($Id_Edu_Almacen){
	...
	try {
        ...
        $html = "<ul class='mensaje_chat' id='mensaje_chat_ul'>";
		$i = 0;
		...
		foreach ($iterator as $key => $item) {
			...
			$Array_Mensajes[$i] = Array(
			"Id_Sala_Conversacion"=>$item['Id_Sala_Conversacion']['S']
			,"Mensaje"=>$item['Mensaje']['S']
			,"Id_Usuario"=>$item['Id_Usuario']['S']
			,"Nombres"=>$item['Nombres']['S']
			,"Fecha_Hora_Creacion"=>$item['Fecha_Hora_Creacion']['S']
			,"Id_Sala"=>$item['Id_Sala']['N']
			);
	    ...
	    array_multisort($Array_Mensajes,SORT_DESC);
        $b  = 0;
        $c  = 0;
		$Array_Mensajes_B = [];		
		foreach ($Array_Mensajes as $itemB) {
			
			$Mensaje = $itemB["Mensaje"];
            $Array_Mensajes_B[$b] = Array(
			"Id_Sala_Conversacion"=>$itemB['Id_Sala_Conversacion']
			,"Mensaje"=>$itemB['Mensaje']
			,"Id_Usuario"=>$itemB['Id_Usuario']
			,"Nombres"=>$itemB['Nombres']
			,"Fecha_Hora_Creacion"=>$itemB['Fecha_Hora_Creacion']
            ,"Id_Sala"=>$itemB['Id_Sala']			
			);
			$b += 1;
			 if (++$c > 30) break;			
		}	
	    ...
	    array_multisort($Array_Mensajes_B,SORT_ASC);				
		foreach ($Array_Mensajes_B as $itemC) {
			$Mensaje = $itemC["Mensaje"];
			$Fecha_Hora_Creacion = $itemC["Fecha_Hora_Creacion"];
			$Id_Sala_Conversacion = $itemC["Id_Sala_Conversacion"];
			$Id_Usuario = $itemC["Id_Usuario"];
			$Nombres = $itemC["Nombres"];
			$Id_Sala = $itemC["Id_Sala"];
            $Fecha_Hora_Creacion_T = trim($Fecha_Hora_Creacion);
			
			$html .= "<li id='chat_".$Id_Sala_Conversacion."' fecha_value ='".$Fecha_Hora_Creacion."'  id_almacen='".$Id_Sala."'  >
			
				<div>".$Mensaje."</div>
				<div ><span><b>".$Nombres."</b> </span><span style='font-size:0.85em;'> |   ".$Fecha_Hora_Creacion."</span></div>
...
/*
function LeerMensajeChatDDB_Ultimo_Mensaje($Id_Edu_Almacen,$Fecha){
	...
	try {
		...
		$return = Array();
		$return["Html"] =$Array_Mensajes_C;
		$return["Nro_Msj"] = $cont_b;
...
function correlativo($Correlativo){
    $Conection = Conection();
		...
		$Query = "
			SELECT 
			SC.Id_Edu_Correlativo
			, SC.Correlativo 
			, SC.Numero_Correlativo 
			FROM edu_correlativo SC
			WHERE 
			SC.Correlativo = :Correlativo  
		";	
		$Where = ["Correlativo" => $Correlativo];
		$Row = ClassPdo::DCRow($Query,$Where,$Conection);	
		$Numero_Correlativo = $Row->Numero_Correlativo;	
		$Id_Edu_Correlativo = $Row->Id_Edu_Correlativo;	
		...
		if(empty($Numero_Correlativo)){
			...
			$Return = ClassPDO::DCInsert("edu_correlativo", $data, $Conection);		
			$lastInsertId = $Return["lastInsertId"] + 1;
			...
			$where = array('Id_Edu_Correlativo' => $Id_Edu_Correlativo);
			$rg = ClassPDO::DCUpdate('edu_correlativo', $reg , $where, $Conection);	
			$lastInsertId = $Numero_Correlativo + 1;
		}
        return $lastInsertId;
}
