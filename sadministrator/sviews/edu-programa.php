    ...
    public  function __construct($Parm=null)
	{
        ...
        switch ($Interface) {
            case "begin":
				...
				$Query="SELECT UM.Version_Desarrollo_Certificado
								...
								$Row = ClassPdo::DCRow($Query,$Where,$Conection);	
				$Version_Desarrollo_Certificado = $Row->Version_Desarrollo_Certificado;	

                if( $Version_Desarrollo_Certificado == 2 ){

                   $Query = " 
						SELECT PA.Id_Edu_Almacen 
						, AR.Id_Edu_Articulo
						,  AR.Nombre
						, AR.Url_Amigable
						, AR.Estado
						...
						FROM programa_alumno PA
						INNER JOIN edu_almacen EA ON PA.Id_Edu_Almacen = EA.Id_Edu_Almacen
						INNER JOIN edu_articulo AR ON AR.Id_Edu_Articulo = EA.Id_Edu_Articulo            
						WHERE 
						PA.Id_User = :Id_User AND  PA.Entity = :Entity AND PA.Id_Programa_Cab = :Id_Programa_Cab 
						AND  PA.Fecha_Fin  >= :Fecha_Fin
						
						";
				   
					$Where = ["Id_User"=>$User,"Entity"=>$Entity,"Id_Programa_Cab"=>$Id_Programa_Cab,"Fecha_Fin"=>$DCDate];
            	...
            	$Query = "
					SELECT  UM.Id_User_Miembro
					FROM user US
					INNER JOIN  user_miembro UM ON UM.Id_User_Creation = US.Id_User
					WHERE 
					US.Id_User =:User
				";	
				$Where = ["User"=>$User];
				...
				$Query = "        
					...
					WHERE EC.Id_Edu_Almacen = :Id_Edu_Almacen
					AND  EC.Id_Suscripcion = :Id_Suscripcion
					AND  EC.Tipo_Producto = :Tipo_Producto
				
				";
				
				
				$Where = ["Id_Edu_Almacen" => $Id_Programa_Cab , "Id_Suscripcion" => $User, "Tipo_Producto" => "programa" ];
				// var_dump($Where);
				$Row = ClassPdo::DCRow($Query,$Where,$Conection);	
					...
					if($Pago_Total == "Realizado" ){
						...
						if($Estado_Academico == "aprobado" || $Estado_Academico == "participado" ){
							...
							if($Estado_Edicion_Datos_Certificado == "Pendiente" ){
								...
								$btn = " Descarga aquí tu certificado ]" .$UrlFile_Certificado ."/interface/Create_Edit_Certificado/Id_Edu_Almacen/".$Id_Programa_Cab."/Id_Edu_Certificado/".$Id_Edu_Certificado."/tipo-producto/programa/interfaceraiz/mostrarcertificado]animatedModal5]HXM]]}";				
							...
							if($Estado_Edicion_Datos_Certificado == "Revisado" ){
								...
								$btn = " Puedes Descargar aquí tu certificado ]/sadministrator/edu-certificado/interface/List_Visor/key/".$Id_Programa_Cab."/Id_Edu_Certificado/".$Id_Edu_Certificado."/tipo-producto/programa/interfaceraiz/mostrarcertificado/request/on/]]HREF]]}";				
				...
				$Contenido = DCPage($DCPanelTitle , $Pestanas . $Button_Mensaje . "<br>" . $Html .  $Plugin ,"panel panel-default");
				$Contenido .= '';
