        ...
        switch ($Interface) {
         	case "":
				...
				$Query = "
					...
					, PC.Id_Programa_Cab AS Codigo
					, PC.Nombre	
					, PC.Estado	
					, PC.Horas_Lectivas	
				 
					, IF( ETT.Nombre IS NULL, 'Programa', ETT.Nombre)  AS TipoTitulo
					, PC.Fecha_Creada
		    ...
		    //1er Page
		        	case "List_programa_det":
						...
						$listMn .= "<i class='icon-chevron-right'></i> Tipos de Titulo [/sadministrator/edu-tipo-titulo/interface/List[animatedModal5[HXM[{";
                        ...
                        if (empty($TipoTitulo)) {						
						    $TipoTitulo = "PROGRAMA EN";
						}
						

						$DCPanelTitle = DCPanelTitle("<b style='color:blue;'> ".$TipoTitulo." </b>".$Nombre."","Estos son los cursos seleccionados | Fecha_Inicio: ".$Fecha_Inicio." | Fecha_Fin: ".$Fecha_Fin,$btn);
		    ...
		    //2do Page
		        	case "List_programa_det_curso":
						...
						$Query = "
							SELECT 
							UM.Id_User_Miembro as CodigoLink,UM.Nombre,UM.Email , SC.Estado as Estado_Curso
							FROM programa_alumno SC
							INNER JOIN programa_det PD ON SC.Id_Edu_Almacen=PD.Id_Edu_Almacen
							INNER JOIN  user_miembro UM ON SC.Id_User = UM.Id_User_Miembro
							INNER JOIN  user US ON UM.Id_User_Creation = US.Id_User
							WHERE 
							SC.Entity = :Entity AND  PD.Id_Programa_Det = :Id_Programa_Det AND SC.Id_Programa_Cab = :Id_Programa_Cab
						";    
						...
						$Screen = 'animatedModal5';
						$where = ["Entity"=>$Entity,"Id_Programa_Det"=>$Id_Programa_Det ,"Id_Programa_Cab"=>$Id_Programa_Cab];
						// var_dump($where);
