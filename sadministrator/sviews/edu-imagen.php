...
class Edu_Imagen{
    ...
    public  function __construct($Parm=null)
	{
		...
		$UrlFile = "/sadministrator/edu-imagen";
        ...
        switch ($interface) {
            ...
            case "y":
				...
				$Contenido = DCPage("" , $Content . $Imagen_Banner . $DCSlidePlugin ,"panel panel-default");
	...
	public function Contenidos_Item($Id_Edu_Almacen,$UrlFile,$Id_Edu_Componente_S,$Id_Suscripcion,$Id_Edu_Area_Conocimiento) {
		...
		$Imagen_Banner = "
				<div id='Screen_Banner'  style='bottom:0px;position:relative;width:100%;' >
		...
		$Url = "http://www.yachai.org/sadministrator/edu_articulo_det/interface/begin/request/on/key/".$Id_Edu_Almacen."/action/Sugerencia";
		...
		$Query = "
			...
			ORDER BY EAR.Date_Time_Creation DESC
			LIMIT 0,2
		";    
		...
		foreach($Datos_Articulos_Vinculados AS $Field){
				...
				$Articulos_Vinculados .='	
								    <tr>
										...
										<td>
										<a href="/sadministrator/edu-imagen/interface/begin/request/on/key/'.$Field->Id_Edu_Almacen.'/action/Sugerencia"  style="color:#000;">
										'.$Field->Nombre.'
										</a>											
										</td>				
								    </tr>					
		...
		";  
		$Where = ["Id_Edu_Area_Conocimiento"=>7, "Estado"=>"Activo","Id_Edu_Tipo_Estructura"=>3];//Blog
		...
		<div class="col-md-12">	
			<div class="col-md-8" >	
				<div class="c-imageB" style="background-image:url('.$Url_Img.');" ></div>
				
				<div>  '.$Field->Contenido_Embebido.'</div>
				<div style="width:100%;padding:10px 0px 0px 0px;">  '.$Paneles_Inferiores.'</div>
			</div>
			<div  class="col-md-4" style="margin-bottom:30px;">'.$Articulos_Vinculados . $Promocion_Productos_Varios.'</div>						
		</div>		
