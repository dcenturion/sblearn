...
class Biblioteca{
	...
	public function Tabs($arg){
		...
		// DCVd($Perfil);
		if($Perfil != 3){
			$menu .= "Producción ]/sadministrator/articulo/request/on?pagina=1]HREF]]}";			
	...
	public function Articulos($Parm,$Tipo_Vista,$desdePaginador , $totalArticulos_x_pagina){
		global $Conection;
		...
		if($Tipo_Vista == "En_Curso"){
			...
			$Query = "
				...
				ORDER BY AR.Date_Time_Creation DESC
				LIMIT ".$desdePaginador.",".$totalArticulos_x_pagina."
		...
		foreach($Rows AS $Field){
					...
					if(empty($Id_Suscripcion)){
						...
						$Date_C = explode(" ", $Field->Date_Time_Creation);
                        ...
                        if($Tipo_Vista == "Creacion" ){	
						
						    $Card .= '<div class="cp-label bg-danger" style="bottom: 0px;width: 100%; background-color: #777573 !important;"> F.C: '.$Date_C[0].'</div>';
						}
