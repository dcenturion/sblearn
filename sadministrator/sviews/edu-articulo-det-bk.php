        ...
        switch ($interface) {
            ...
            case "y":
					...
					if($Id_Edu_Tipo_Privacidad == 3){
						...
						if(!empty($User)){	
						    ...
						    if($Descarga == "SI"){
								...
								$btn = "<i class='zmdi zmdi-download zmdi-hc-fw'></i> Descargar ".$Descarga."  ]" .$Dominio_url."]]HREF_DONWLOAD]]btn btn-primary m-w-120}";
								$btn = DCButton($btn, 'botones1', 'sys_form'.$Count);
				...
				if($Perfil == 1 || $Perfil == 2 || $Perfil == 777){
					...
					$listMn .= " Generar Url del curso[".$UrlFile_Edu_Blog."/interface/Generar_Url_Amigable/Id_Edu_Articulo/".$Id_Edu_Articulo."/key/".$Id_Edu_Almacen."/Id_Edu_Componente_S/".$Id_Edu_Componente_S."[animatedModal5[HXM[{";
					$listMn .= "<i class='zmdi zmdi-edit zmdi-hc-fw'></i> Metodo de Evaluar [".$UrlFile."/interface/Configurar_Evaluar/key/".$Id_Edu_Almacen."[animatedModal5[HXM[{";
					...
					$btn = "<i class='zmdi zmdi-menu zmdi-hc-fw'></i> Configuracion ]SubMenu]{$listMn}]OPCIONES]]btn-simple-c}";
					...
					$listMn .= " Inscripción Masiva de Participante [".$UrlFile_Edu_Participante_Masivo."/interface/List/key/".$Id_Edu_Almacen."[animatedModal5[HXM[{";
					$listMn .= "  Gestión de Certificado [".$UrlFile_Edu_Gestion_Certificado."/interface/List/key/".$Id_Edu_Almacen."/request/on[_blank[HREF[{";
				...
				///Este es el listado del chat que se imprime pero se mantine oculto
				///La funcion MonitorChat(), captura el ultimo LI que contiene la fecha que se compara contra la base de datos				
				//$Mensajes =  LeerMensajeChatDDB($Id_Edu_Almacen);
				$Chat = " 
				...
				$Chat .=' <button type="button"  id="boton_chat" onclick="SaveFormSP(this);" 
				...
				if(!empty($Horas_Lectivas)){
					...
					if($Pago_Total == "Realizado" ){
						...
						if($Estado_Academico == "aprobado" || $Estado_Academico == "participado" ){
								...
								// echo "<br>".$Horas_Lectivas;
							if($Estado_Edicion_Datos_Certificado == "Pendiente" ){
								
								$btn = " Descarga aquí tu certificado ]" .$UrlFile_Certificado ."/interface/Create_Edit_Certificado/Id_Edu_Almacen/".$Id_Edu_Almacen."/Id_Edu_Certificado/".$Id_Edu_Certificado."]animatedModal5]HXM]]}";				
							...
							if($Estado_Edicion_Datos_Certificado == "Revisado" ){
								...
								$btn = " Puedes Descargar aquí tu certificado ]/sadministrator/edu-certificado/interface/List_Visor/key/".$Id_Edu_Almacen."/Id_Edu_Certificado/".$Id_Edu_Certificado."/request/on/]]HREF]]}";				
				...
				$Layout = array(array("PanelA","col-md-8",$PanelA),array("PanelB","col-md-4",$Chat . $Imagen_Banner.$DCSlidePlugin.$PanelB));
            ...
            case "Elementos":
				...
				if(!empty($User)){
							// $btn = "<i class='zmdi zmdi-comments zmdi-hc-fw'></i> Chat <div id='alerta_chat'> </div>]" .$UrlFile."/interface/Chat_GrupalB/key/".$Id_Edu_Almacen."/Id_Edu_Componente_S/".$Id_Edu_Componente_S."]PanelB]HXM]]btn-simple}";
							// $btn_chat = DCButton($btn, 'botones1', 'sys_formB'.$Count);	
				...
				if($Id_Edu_Formato == 2){
					...
					if($Id_Edu_Tipo_Privacidad == 3){
                        ...
                        if($Descarga == "SI"){
							...
							$btn = "<i class='zmdi zmdi-download zmdi-hc-fw'></i> Descargar ]" .$Dominio_url."]]HREF_DONWLOAD]]btn btn-primary m-w-120}";
							$btn = DCButton($btn, 'botones1', 'sys_form'.$Count);		
					    }						
					}			
